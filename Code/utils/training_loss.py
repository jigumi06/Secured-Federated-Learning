...
def train_loss_show(args, loss_dict, clients_index):
    ...
    plt.savefig('results/ASHE/loss.png')
    #plt.show() 
...
def train_globalacc_show(args, globalacc_list):
    fig1, ax1 = plt.subplots()
    E_list = [i + 1 for i in range(args.r)]
    ax1.plot(E_list, globalacc_list, label=('mean global accuracy'))
    ax1.set_title('mean global accuracy vs. round')
    ax1.set_ylabel('accuracy')
    ax1.set_xlabel('round')
    ax1.legend()
    
    plt.savefig('results/ASHE/global_acc.png')
    #plt.show()
