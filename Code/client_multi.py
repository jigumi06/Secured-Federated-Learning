...
def start_clients(args, client_index, anchorloss_funcs, client_models, global_model, global_round, dataset_train, dict_users, loss_dict, masks, layer_list, shape_list, isAuthentic):
    if isAuthentic and global_round != 0:
        ...
        for k in client_index:
            thread = threading.Thread(target=decrypt_by_para, args=(args, global_round, layer_list[k], shape_list, k, global_model, client_models[k], decryption_queue))
        ...
        while not encryption_queue.empty():
            k, layer_list = encryption_queue.get()
            layer_list[k] = layer_list
        '''
    else:
        for k in client_index:
...
def decrypt_by_para(args, global_round, layer_list, shape_list, global_model):
    #print("<Enter decrypt>")
   # if global_round != 0 :
    #decrypted_layers = []
    layer_count = 0
    ...
    #client_model = copy.deepcopy(global_model)
        ...
        #     for param, new_data in zip(model.parameters(), reshaped_tensors):
